...
def test_session5_time_it_print():
    """  Measuring time taken by print function for 5 iterations   """
    total_time_taken = session5.time_it(print,"How are you", repetitions=5)    
    assert (total_time_taken<0.1 and total_time_taken>0), "Implementation takes more than normal time or no time"


def test_session5_time_it_squared_power_list():
    """ Measuring time taken by squared_power_list function for 5 iterations    """    
    total_time_taken = session5.time_it(session5.squared_power_list,1,start=1,end=5, repetitions=5)
    assert (total_time_taken<0.1 and total_time_taken>0), "Implementation takes more than normal time or no time"



def test_session5_time_it_polygon_area():
    """ Measuring time taken by polygon_area function for 5 iterations    """    
    total_time_taken = session5.time_it(session5.polygon_area,10,sides=3, repetitions=5)
    assert (total_time_taken<0.1 and total_time_taken>0), "Implementation takes more than normal time or no time"

def test_session5_time_it_temp_converter():
    """   Measuring time taken by temp_converter function for 5 iterations      """
    total_time_taken = session5.time_it(session5.temp_converter,25,temp_given_in='c', repetitions=5)
    assert (total_time_taken<0.1 and total_time_taken>0), "Implementation takes more than normal time or no time"


def test_session5_time_it_speed_converter():
    """ Measuring time taken by speed_converter function for 5 iterations       """
    total_time_taken = session5.time_it(session5.speed_converter,90,dist='km', time='min', repetitions=5)
    assert (total_time_taken<0.1 and total_time_taken>0), "Implementation takes more than normal time or no time"
...
def test_session5_polygon_area_length():
    ...
    with pytest.raises(TypeError, match=r".*integer is expected for length*"):
        session5.polygon_area('five',sides=2)
    with pytest.raises(TypeError, match=r".*integer is expected for length*"):
        session5.polygon_area(1+1j,sides=2)
...
def test_session5_polygon_area_sides():
    ...
    with pytest.raises(TypeError, match=r".*integer is expected for sides*"):
        session5.polygon_area(5,sides='ten')
    with pytest.raises(TypeError, match=r".*integer is expected for sides*"):
        session5.polygon_area(5,sides=1+1j)

def test_session5_polygon_area_sides_values():
    """Test polygon_area function for permissible values for sides, check for 0, 1, 2, 7"""
    length = 10
    with pytest.raises(TypeError, match=r".*Invalid sides value for a polygon*"):
...
def test_session5_polygon_area_length_values():
    ...
    length = 8
    assert math.isclose(
        64, session5.polygon_area(length, sides=4), rel_tol=1e-3
    ), "test_session5_polygon_area fails for 7 sides"
...
def test_session5_temp_converter_temp():
    ...
    with pytest.raises(TypeError, match=r".*integer or float is expected*"):
        session5.temp_converter('five',temp_given_in='c')

    with pytest.raises(TypeError, match=r".*integer or float is expected*"):
        session5.temp_converter(1+1j,temp_given_in='C')
